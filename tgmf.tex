...
试件外表面为温度边界，随时间变化的温度分布可由试验测得，试件内表面为对流边界条件，对流换热系数由圆管强制对流换热公式计算，需要试验测量冷却空气的密度、温度和体积流量。由于试件的对称性，中心对称界面可假设为绝热边界，试件的上边界认为是热传导边界，对于热辐射，我们去试件表面的发射率$\varepsilon=0.75$。
...
图\ref{Fig:plot_temperature_along_gauge_length}为试件内表面和外表面温度与
\begin{figure}[!htp]
